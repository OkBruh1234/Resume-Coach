        ...
        try:
            ...
            if "RESOURCE_EXHAUSTED" in str(e) or "429" in str(e):
                if attempt < max_retries - 1:
                    print(f"RATE LIMIT HIT (429). Sleeping for 15 seconds before attempt {attempt + 2}...")
                    time.sleep(15)
                    continue
            # If not a rate limit, or we ran out of retries, raise it
            return {
                "ats_score": "Unknown",
                "gap_analysis": {
                    "missing_keywords": [],
                    "suggestions": f"Critical AI Pipeline Error after retries: {e}"
                }
            }
